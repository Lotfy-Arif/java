...
//            out.println( "<br>" );
            ...
            out.printf("<select name=\"%s\" id=\"PRIVATELY_OWNED\">", BloodBankLogic.PRIVATELY_OWNED);
            out.println("<option value=\"true\">Positive</option>");
            out.println("<option value=\"false\">Negative</option>");
            out.println("</select><br><br>");
            
            BloodBankLogic drLogic = LogicFactory.getFor( "BloodBank" );
            String date = drLogic.convertDateToString(Calendar.getInstance().getTime());
            
            out.println( "Established:<br>" );
            out.printf( "<input type=\"text\" name=\"%s\" value=\"%s\"><br>", BloodBankLogic.ESTABLISHED, date );
            out.println( "<br>" );
    ...
    protected void doPost( HttpServletRequest request, HttpServletResponse response )
            throws ServletException, IOException {
        log( "POST" );
        BloodBankLogic zLogic = LogicFactory.getFor( "BloodBank" );
        String name = request.getParameter( BloodBankLogic.NAME );
        if( zLogic.getBloodBankWithName( name ) == null ){
            try {
                BloodBank bloodbank = zLogic.createEntity( request.getParameterMap() );
                zLogic.add( bloodbank );
            } catch( Exception ex ) {
